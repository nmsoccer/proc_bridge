...
#BRIDGE_USER="soullei"
#BRIDGE_DIR="/home/soullei/proc_bridge"
BRIDGE_USER=""
BRIDGE_DIR=""
		...
		if cmp(title , 'PROC') == 0:
			...
			while True:
				...
				if int(proc_id) <= MANAGER_PROC_ID_MAX:
					...
					return -1;
				if proc_id_check.has_key(proc_id):
		...
		#handle [channel]
			...
			while True:
				...
				for proc_name in proc_names:
					if proc_dict.has_key(proc_name) == False:
					...
					if proc_info.has_key("target_list"):
					...
					for target in content:
						if proc_dict.has_key(target) == False:
...
def mix_manager_and_proc():
	...
	for proc_name in proc_dict:
		proc_info = proc_dict[proc_name];
		if not proc_info.has_key('target_list'):
			proc_info['target_list'] = [];
		target_list = proc_info['target_list'];
		for manager_name in manager_dict:
	...
	for manager_name in manager_dict:
		proc_info = manager_dict[manager_name];
		if not proc_dict.has_key(manager_name):
...
def parse_pattern(src_content):
	#print ">>>>try to parse: " +src_content
	result = [];
	prefix = ""
	rest = ""
	content = src_content;
...
def build_one_bridge(proc_name):
	...
	if not proc_dict.has_key(proc_name):
...
def build_one_bridge_old(proc_name):
	...
	proc_port = proc_info["proc_port"]
	target_list = []
	if proc_info.has_key("target_list") == False: #no target
#		pass
		return 0
	else:
		target_list = proc_info["target_list"]
